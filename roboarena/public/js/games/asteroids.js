...
window.startGame = function() {
  ...
  function update() {
    ...
    // BUGFIX: the previous version called bullets.splice()/asteroids.splice()
    // from *inside* a bullets.forEach(...) -> asteroids.forEach(...) nested
    // loop. Mutating an array while forEach is iterating it makes forEach
    // skip the element that just shifted into the removed slot — so some
    // real hits were silently missed, and in rarer cases a bullet could be
    // double-credited against an asteroid that had already been spliced out
    // from under it. Fixed with a mark-then-filter pattern: nothing is
    // removed from `bullets`/`asteroids` until both loops have finished.
    const hitBullets = new Set();
    const hitAsteroids = new Set();
    const spawned = [];
    bullets.forEach((b, bi) => {
      if (hitBullets.has(bi)) return;
      asteroids.forEach((a, ai) => {
        if (hitBullets.has(bi) || hitAsteroids.has(ai)) return;
        if (Math.hypot(b.x-a.x,b.y-a.y) < a.r) {
          ...
          if (a.r > 22) {
            for(let i=0;i<2;i++){
              ...
              spawned.push({...a, r:a.r/2, vx:Math.cos(angle)*sp, vy:Math.sin(angle)*sp,
    ...
    if (hitBullets.size) bullets = bullets.filter((_, bi) => !hitBullets.has(bi));
    if (hitAsteroids.size) asteroids = asteroids.filter((_, ai) => !hitAsteroids.has(ai));
    if (spawned.length) asteroids = asteroids.concat(spawned);
