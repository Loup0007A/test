...
// Wired for Arena.robotMode: when an admin is live, the robot's "reaction
// time" is the admin's real click instead of a random timer.
window.startGame = function() {
  ...
  if (live) {
    Arena.robotMode.onAction(function(type, payload) {
      if (type === 'admin-clicked' && phase === 'ready') {
        phase = 'done';
        robotTime = payload.reactionMs;
        losses++;
        document.getElementById('qd-time').innerHTML = `<span style="color:var(--danger)">🤖 Le robot (humain) a gagné (${Math.floor(robotTime)}ms)</span>`;
  ...
  function nextRound() {
    ...
    document.getElementById('qd-round').textContent = `Round ${round}/${ROUNDS} — Victoires: ${wins} | Défaites: ${losses}` + (live ? ' — 🔴 Robot humain en direct' : '');
    ...
    waitTimeout = setTimeout(() => {
      ...
      if (live) {
        // Not answered yet: treat as "infinitely slow" until the admin
        // actually clicks, so a player who clicks first always wins that race.
        robotTime = Infinity;
        Arena.robotMode.send('go', {});
      } else {
  ...
  document.getElementById('qd-btn').addEventListener('click', () => {
    if (phase === 'wait') {
      ...
      playerTime += reaction;
      if (live) Arena.robotMode.send('player-clicked', { reactionMs: reaction });
