...
// Wired for Arena.robotMode: when an admin is live, their armed move
// (chosen ahead of time in the admin panel) replaces the prediction AI.
window.startGame = function() {
  ...
  let adminMove = 'rock'; // matches the admin panel's default-armed move

  if (live) {
    Arena.robotMode.onAction(function(type, payload) {
      if (type === 'arm-move' && ['rock', 'paper', 'scissors'].includes(payload.move)) {
        adminMove = payload.move;
      }
    });
  }
  ...
  c.innerHTML = `
    <div style="text-align:center;padding:24px;width:100%;max-width:480px">
      <div id="rps-scores" style="display:flex;justify-content:space-around;font-family:var(--font-display);margin-bottom:16px">
        ...
        <div><div style="font-size:0.9rem;color:var(--text3);padding-top:8px">Round <span id="rps-r">1</span>/${ROUNDS}${live ? ' — 🔴 direct' : ''}</div></div>
  ...
  window.rpsPlay = function(playerMove) {
    ...
    round++;
    const robotMove = live ? adminMove : robotPredict();
    history.push(playerMove);

    if (live) Arena.robotMode.send('move-result', { playerMove, robotMove });
