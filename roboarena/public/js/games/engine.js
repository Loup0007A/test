...
// ── Inline toast (don't depend on main.js load order) ──
function showToast(message, type) {
  // Use global showToast if available (from main.js), else fallback
  if (window._showToast) { window._showToast(message, type); return; }
  ...
  if (!document.getElementById('_toastStyle')) {
    ...
    s.id = '_toastStyle';
    s.textContent = '@keyframes toastIn{from{transform:translateX(100px);opacity:0}to{transform:translateX(0);opacity:1}}';
...
const Arena = {
  ...
  async end(result, score) {
    this.running = false;
    this.score = (score !== undefined && score !== null) ? score : this.score;
...
// Default startGame — overridden by each game's script
