...
router.get('/:gameId', optionalAuth, async (req, res, next) => {
  try {
    ...
    const leaderboard = await Scores.getLeaderboard(game.id, 10);
...
router.post('/:gameId/result', requireAuth, async (req, res, next) => {
  try {
    const game = getGameById(req.params.gameId);
    ...
    await Users.recordGame(req.user.id, result === 'win');
    log('game_result', { userId: req.user.id, gameId: game.id, result, score });

    res.json({
      success: true,
      message: result === 'win' ? '🏆 Victoire !' : result === 'draw' ? '🤝 Égalité' : '🤖 Le robot gagne',
    });
  } catch (err) {
    next(err);
  }
