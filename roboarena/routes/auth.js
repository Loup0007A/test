...
const { Users, Scores, log } = require('../models/db');
...
router.post('/register', registerLimiter, async (req, res, next) => {
  try {
    ...
    // Duplicate username/email inserted concurrently between the check
    // above and the INSERT triggers the unique constraint — handle it
...
router.get('/profile', requireAuth, async (req, res, next) => {
  try {
    const scores = (await Scores.getByUser(req.user.id)).slice(-20).reverse();
